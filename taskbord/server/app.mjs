...
import {
  CODEX_USAGE_DEFAULT_RANGE_DAYS,
  CODEX_USAGE_MAX_RANGE_DAYS,
  CodexUsageService,
...
const EMBEDDED_CHALLENGE_QUERY = "__codex_taskboard_challenge";
...
function parseUsageRangeDays(value) {
  const rangeDays = Number(value);
  if (!Number.isSafeInteger(rangeDays) || rangeDays < 1 || rangeDays > CODEX_USAGE_MAX_RANGE_DAYS) {
    throw new ApiError(
      400,
      "INVALID_QUERY_PARAMETER",
      `'rangeDays' must be an integer from 1 to ${CODEX_USAGE_MAX_RANGE_DAYS}`,
    );
  }
  return rangeDays;
}
...
function parseTaskCreate(body) {
  ...
  if (!hasTaskConversationForStatus(task.status, task.threadId)) {
    throw new ApiError(
      409,
...
async function discoverMcpServers(codexExecutable, processEnv) {
  const result = await execFileAsync(codexExecutable, ["mcp", "list", "--json"], codexExecFileOptions(codexExecutable, {
    env: processEnv,
    timeout: 8_000,
...
export function createTaskboardServer(options = {}) {
  ...
  const server = createServer(async (request, response) => {
    ...
    try {
      ...
      const origin = request.headers.origin;
      const challengeHeader = request.headers["x-codex-taskboard-challenge"];
      const challenge = typeof challengeHeader === "string"
        ? challengeHeader
        : url.searchParams.get(EMBEDDED_CHALLENGE_QUERY);
      ...
      if (
        ...
        && origin === "app://-"
        && (!isStaticRequest || challenge !== null)
      ) {
      ...
      url.searchParams.delete(EMBEDDED_CHALLENGE_QUERY);
      ...
      const projectUsageRoute = pathname.match(/^\/api\/local\/projects\/([^/]+)\/usage$/);
      if (projectUsageRoute) {
        if (request.method !== "GET") return methodNotAllowed(response, ["GET"]);
        assertAllowedQuery(url.searchParams, new Set(["rangeDays"]), "GET /api/local/projects/:id/usage");
        const projectId = validateProjectId(
          decodeRouteSegment(projectUsageRoute[1], "Project id"),
        );
        const rangeDays = parseUsageRangeDays(
          url.searchParams.get("rangeDays") ?? String(CODEX_USAGE_DEFAULT_RANGE_DAYS),
        );
        const usage = await codexUsage.get(projectId, rangeDays);
        if (!usage) throw new ApiError(404, "PROJECT_NOT_FOUND", `Project '${projectId}' was not found`);
