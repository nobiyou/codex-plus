...
export function parseTaskboardAutomationHostRequest(value) {
  ...
  if (value.targetThreadId !== undefined && !validIdentifier(value.targetThreadId, 128)) return null;
  ...
  return {
    ...
    ...(value.targetThreadId === undefined ? {} : { targetThreadId: value.targetThreadId }),
...
function sanitizeAutomation(item, request) {
  const model = item?.model ?? request?.model;
  const reasoningEffort = item?.reasoningEffort ?? request?.reasoningEffort;
  const targetThreadId = item?.targetThreadId;
  if (
    !validText(item?.id, 256)
    || (item.status !== "ACTIVE" && item.status !== "PAUSED")
    || !isSupportedModelEffort(model, reasoningEffort)
    || !validRrule(item.rrule)
    || (targetThreadId !== undefined && !validIdentifier(targetThreadId, 128))
  ) return null;
  return {
    id: item.id,
    status: item.status,
