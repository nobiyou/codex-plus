...
test("project usage reads matching Codex sessions and validates the range", async () => {
  let workspacePath;
  const baseUrl = await startServer(async (directory) => {
    workspacePath = path.join(directory, "usage-workspace");
    ...
    const matchingSession = [
      ...
      },
      {
        ...
        type: "turn_context",
        payload: { workspace_roots: [workspacePath] },
      },
      {
      ...
      },
      {
        ...
        payload: {
          ...
          thread_settings: { model: "gpt-usage" },
        },
      },
      {
        timestamp: new Date(now).toISOString(),
    ...
    const unrelatedSession = [
      {
        timestamp: new Date(now).toISOString(),
        type: "session_meta",
        payload: { cwd: path.join(directory, "other-workspace"), model: "gpt-unrelated" },
      },
      {
        timestamp: new Date(now).toISOString(),
        type: "event_msg",
    ...
    await writeFile(
      path.join(sessionsDirectory, "rollout-2026-08-12-019ff641-afef-7052-941e-0f0b08b3ef88.jsonl"),
      `${matchingSession.map((record) => JSON.stringify(record)).join("\n")}\n`,
    );
    await writeFile(
      path.join(sessionsDirectory, "rollout-2026-08-12-019ff641-afef-7052-941e-0f0b08b3ef89.jsonl"),
      `${unrelatedSession.map((record) => JSON.stringify(record)).join("\n")}\n`,
    );
    return { codexStatePath: path.join(directory, "codex-state.json") };
  });

  const projectResult = await request(baseUrl, "/api/projects", {
    method: "POST",
    body: { id: "usage-project", name: "Usage project", workspacePath },
  });
  assert.equal(projectResult.response.status, 201);

  const usageResult = await request(baseUrl, "/api/local/projects/usage-project/usage?rangeDays=7");
  assert.equal(usageResult.response.status, 200);
  assert.equal(usageResult.body.projectId, "usage-project");
  assert.equal(usageResult.body.rangeDays, 7);
  assert.equal(usageResult.body.sessions, 1);
  assert.deepEqual(usageResult.body.totals, {
    inputTokens: 110,
  ...
  assert.deepEqual(usageResult.body.models.map((model) => model.model), ["gpt-usage"]);

  const invalidRange = await request(baseUrl, "/api/local/projects/usage-project/usage?rangeDays=0");
  assert.equal(invalidRange.response.status, 400);
  assert.equal(invalidRange.body.error.code, "INVALID_QUERY_PARAMETER");

  const missingProject = await request(baseUrl, "/api/local/projects/missing-project/usage");
