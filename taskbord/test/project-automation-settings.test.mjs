...
test("opening settings and changing projects reconcile with the host list", () => {
  ...
  assert.match(
    reconcileSource,
    /saveRevision !== automationSaveRevisionRef\.current\s*\|\|\s*queuedAutomationSavesRef\.current\.has\(projectId\)/,
  );
  assert.match(appSource, /const automationRequestContextKey = automationRequestContext\s*\n\s*\? JSON\.stringify\(automationRequestContext\)/);
  assert.match(appSource, /reconcileProjectAutomationRef\.current\(\)/);
  assert.match(appSource, /\[selectedProjectId, automationRequestContextKey\]/);
});
