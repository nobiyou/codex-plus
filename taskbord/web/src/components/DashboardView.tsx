...
import { getProjectSummary, getProjectUsage } from "../api";
...
const USAGE_MODEL_COLOR_MAP: Record<string, string> = {
  "gpt-5.6-sol": "#c6dcf7",
  "gpt-5.5": "#3d82e8",
  "gpt-5.4": "#294e91",
  "gpt-5.6-luna": "#c2a8ee",
  "gpt-5.6-terra": "#804de0",
  "codex-auto-review": "#e866a9",
  "gpt-image-2": "#f1c95b",
  "gpt-5.4-mini": "#ec7c3e",
  unknown: "#aab2bf",
};
const USAGE_MODEL_FALLBACK_COLORS = ["#22b889", "#d25e5e", "#5482d2", "#bf49d7", "#e98155"];
...
function usageDayLabel(day: CodexUsageDay, locale: string) {
  return new Intl.DateTimeFormat(locale, { month: "short", day: "numeric" })
    .format(new Date(`${day.date}T12:00:00`));
}

function usageModelLabel(model: CodexUsageModel) {
  return model.model === "unknown" ? "Unknown model" : model.model;
}

function usageModelColor(model: string) {
  const knownColor = USAGE_MODEL_COLOR_MAP[model];
  if (knownColor) return knownColor;
  let hash = 0;
  for (const character of model) hash = (hash * 31 + character.charCodeAt(0)) | 0;
  return USAGE_MODEL_FALLBACK_COLORS[Math.abs(hash) % USAGE_MODEL_FALLBACK_COLORS.length];
}
...
export function DashboardView({
  ...
  const [projectUsage, setProjectUsage] = useState<CodexUsageSummary | null>(null);
  ...
  useEffect(() => {
    ...
    async function loadUsage() {
      try {
        const next = await getProjectUsage(projectId, 30, controller.signal);
        if (disposed) return;
  ...
  const usageDays = projectUsage?.days ?? [];
  const usageMaximum = Math.max(1, ...usageDays.map((day) => day.totals.totalTokens));
  const usageModels = projectUsage?.models ?? [];
  const usageTotal = projectUsage?.totals.totalTokens ?? 0;
  const usageModelOrder = new Map(usageModels.map((model, index) => [model.model, index]));
      ...
      <div className="dashboard-content">
        ...
        <div className="dashboard-grid">
          <section className="dashboard-panel dashboard-usage-panel">
            ...
            {projectUsage ? (
              <div className="dashboard-usage-body">
                <div className="dashboard-usage-summary">
                  <div className="dashboard-usage-total">
                    <small>{text("总 Token", "Total tokens")}</small>
                    <strong>{formatTokenCount(usageTotal)}</strong>
                    <span>{text(
                      `${projectUsage.sessions} 个 session · 输入 ${formatTokenCount(projectUsage.totals.inputTokens)} · 输出 ${formatTokenCount(projectUsage.totals.outputTokens)}`,
                      `${projectUsage.sessions} sessions · input ${formatTokenCount(projectUsage.totals.inputTokens)} · output ${formatTokenCount(projectUsage.totals.outputTokens)}`,
                    )}</span>
                  </div>
                  <div className="dashboard-usage-models" aria-label={text("模型图例", "Model legend")}>
                    {usageModels.length ? usageModels.map((model) => (
                      <div className="dashboard-usage-model" key={model.model}>
                        <i style={{ background: usageModelColor(model.model) }} />
                        <span title={usageModelLabel(model)}>{usageModelLabel(model)}</span>
                        <strong>{formatTokenCount(model.totals.totalTokens)} <em>{usagePercent(model.totals.totalTokens, usageTotal)}%</em></strong>
                      </div>
                    )) : (
                      <span className="dashboard-usage-muted">{text("暂无模型数据", "No model data")}</span>
                    )}
                  </div>
                </div>
                <div className="dashboard-usage-chart" aria-label={text("按日 Token 使用量", "Daily token usage")}>
                  {usageDays.map((day, dayIndex) => {
                    const dayModels = [...day.models].sort((left, right) => (
                      (usageModelOrder.get(left.model) ?? Number.MAX_SAFE_INTEGER)
