...
export function TaskDetail({
  ...
  const [propertyMenu, setPropertyMenu] = useState<
    "status" | "priority" | "assignee" | "labels" | "development" | "recurrence" | "conversation" | null
  >(null);
  const [savingProperty, setSavingProperty] = useState<string | null>(null);
  const [bindingConversation, setBindingConversation] = useState(false);
  ...
  async function bindConversation(threadId: string) {
    const conversation = recentConversations.find((candidate) => candidate.threadId === threadId);
    if (!conversation || currentTask.threadBinding || currentTask.threadId || currentTask.legacyLocalThreadId) return;
    setBindingConversation(true);
    onError(null);
    try {
      const saved = await onBindConversation(currentTask, conversation);
      setCurrentTask(saved);
    } catch (error) {
      onError(issueMessageFor(error));
    } finally {
      setBindingConversation(false);
    }
  }
          ...
          <div className="issue-detail-main">
            <article className="issue-editor" aria-label={text("议题内容", "Issue content")}>
              <div className="issue-editor-content">
                ...
                {!currentTask.threadBinding && !currentTask.threadId && !currentTask.legacyLocalThreadId && (
                  <div
                    className="issue-conversation-list issue-conversation-picker"
                    aria-label={text("绑定处理此议题的已有对话", "Bind an existing conversation to this issue")}
                  >
                    <TaskPropertyPicker
                      value=""
                      options={[
                        {
                          value: "",
                          label: recentConversations.length > 0
                            ? text("绑定已有对话", "Bind existing conversation")
                            : text("暂无可绑定对话", "No conversations available"),
                          icon: <TaskboardIcon name="conversation" />,
                        },
                      ...
                      open={propertyMenu === "conversation"}
                      disabled={bindingConversation || recentConversations.length === 0}
                      className="detail-conversation-picker"
                      triggerClassName="detail-conversation-trigger"
                      ariaLabel={text("绑定已有对话", "Bind existing conversation")}
                      onOpenChange={(open) => setPropertyMenu(open ? "conversation" : null)}
                      onChange={(threadId) => void bindConversation(threadId)}
                    />
                  </div>
                )}
              </div>
