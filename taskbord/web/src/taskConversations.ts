...
export function preferredTaskThreadId(
  task: Pick<Task, "threadId" | "conversationRefs">,
): string | null {
  if (task.threadId) return task.threadId;
  return [...(task.conversationRefs ?? [])]
    .filter((ref) => ref.source === "comment" && normalizeCodexThreadId(ref.threadId))
    .sort((left, right) => right.updatedAt.localeCompare(left.updatedAt))[0]?.threadId ?? null;
}
